...
Poses per session capped at **1000** (best-resolution first). Symmetry-equivalent copies of a ligand within a structure are collapsed to one representative pose. `off-site` = correct location test failed (wrong site/cofactor/lipid); `other` = no matching pocket / unavailable (`poor_fit`, `no_pocket`, `not_found`, `align_error`).
